from data_func.iostream import load_RawObs_training_data,load_RawObservation, load_NA_GeoLatLon, load_NA_GeoLatLon_Map, load_BLISCO_data,load_mahalanobis_distance_data
from data_func.calculation import calculate_covariance_matrix, invert_matrix,calculate_mahalanobis_distance
from data_func.utils import neighbors_haversine_indices,Obs_version,sites_number_dict
...
def Get_absolute_uncertainty_map(species,version,special_name,YYYY,MM,obs_version,nearby_sites_number,
    ...
    if MM != 12:
        ...
        padded_map[5:5995,5:12995] = map_data
    else:
        padded_map = np.zeros(rRMSE_uncertainty_map.shape,dtype=np.float64)
        for m in range(12):
...
def Get_nearby_sites_indices_map(species,version,nearby_sites_number,YYYY:np.int32,MM:np.int32):
    ...
    if MM != 12:
        ...
        for ix in range(NA_GeoLAT_MAP.shape[0]):
            land_index = np.where(landtype[ix,:] != 0)
            print('It is procceding ' + str(np.round(100*(ix/NA_GeoLAT_MAP.shape[0]),2))+'%.' )
            if len(land_index[0]) == 0:
                print('No lands.')
                None
            else:
                idx = neighbors_haversine_indices(
                            obs_lat[nonan_index], obs_lon[nonan_index], NA_GeoLAT_MAP[ix,land_index[0]], NA_GeoLON_MAP[ix,land_index[0]], nearby_sites_number
                        )
                idx = np.array(idx)
...
def Get_local_reference_for_channels(channel_lists,species,version,NA_CNN_version,NorthAmerica_PM25_special_name,
    ...
    if MM != 12:
        ...
        RawObs_training_data = load_RawObs_training_data(channel_lists=channel_lists,
                                                        ...
                                                        start_year=1998,end_year=end_year,width=Width,height=Height)
...
def Calculate_Mahalanobis_distance(channel_lists,species,version,NA_CNN_version,NorthAmerica_PM25_special_name,
    ...
    if MM != 12:
        ...
        total_sites_number = sites_number_dict[species]
        ...
        input_file_dic = inputfiles_table(YYYY=YYYY,MM=MONTH[MM],NorthAmerica_PM25_version=NA_CNN_version,
                                        NorthAmerica_PM25_special_name=NorthAmerica_PM25_special_name)
